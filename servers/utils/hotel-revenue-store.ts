...
import { ensureHotelDailySchema, hotelDailyStore, upsertHotelDailyRecord } from './hotel-daily-store'
...
export const hotelRevenueStore = {
  async list(date?: string, companyId?: unknown) {
    const normalizedCompanyId = normalizeScope(companyId).companyId
    ...
    if (db) {
      ...
      const [rows] = await db.query<mysql.RowDataPacket[]>(
        ...
        `,
        date ? [normalizedCompanyId, date] : [normalizedCompanyId],
      )
  ...
  async listPage(date: string | undefined, current: unknown, pageSize: unknown, companyId?: unknown) {
    const page = normalizePagination(current, pageSize)
    const normalizedCompanyId = normalizeScope(companyId).companyId
    const db = getMysqlPool()
    if (db) {
      await ensureHotelRevenueSchema(db)
      const where = `company_id = ? AND deleted_at IS NULL${date ? ' AND revenue_date = ?' : ''}`
      const params = date ? [normalizedCompanyId, date] : [normalizedCompanyId]
  ...
  async replaceByDate(date: string, rows: unknown, companyId?: unknown, deptId?: unknown, userId?: unknown) {
    ...
    if (db) {
      ...
      await withMysqlTransaction(db, async (connection) => {
        await connection.execute('UPDATE hotel_revenue SET deleted_at = NOW() WHERE company_id = ? AND revenue_date = ? AND deleted_at IS NULL', [scope.companyId, date])
        for (const row of nextRows) {
          await connection.execute(`
          INSERT INTO hotel_revenue (id, company_id, dept_id, owner_user_id, created_by, revenue_date, revenue_json, created_at, updated_at, deleted_at)
          VALUES (?, ?, ?, ?, ?, ?, CAST(? AS JSON), NOW(), NOW(), NULL)
          ON DUPLICATE KEY UPDATE company_id = VALUES(company_id), dept_id = VALUES(dept_id), owner_user_id = VALUES(owner_user_id), revenue_date = VALUES(revenue_date), revenue_json = VALUES(revenue_json), updated_at = NOW(), deleted_at = NULL
        `, [row.id, scope.companyId, scope.deptId, scope.ownerUserId, scope.createdBy, date, JSON.stringify(row)])
        }
      })
      return nextRows
    }
    ...
    const current = await this.list(undefined, scope.companyId)
    const data = [...nextRows, ...current.filter(row => row.date !== date)]
    writeJsonFile(dataFile, data)
    return nextRows
  },

  async applyChanges(date: string, input: { upsert?: unknown, deleteIds?: unknown, daily?: unknown }, companyId?: unknown, deptId?: unknown, userId?: unknown) {
    ...
    if (db) {
      ...
      await withMysqlTransaction(db, async (connection) => {
        for (const id of deleteIds) {
          await connection.execute('UPDATE hotel_revenue SET deleted_at = NOW() WHERE id = ? AND company_id = ? AND revenue_date = ? AND deleted_at IS NULL', [id, scope.companyId, date])
        ...
        for (const row of upsert) {
          await connection.execute(`
            INSERT INTO hotel_revenue (id, company_id, dept_id, owner_user_id, created_by, revenue_date, revenue_json, revenue_type, category, amount, payment_method, handler, created_at, updated_at, deleted_at)
            VALUES (?, ?, ?, ?, ?, ?, CAST(? AS JSON), ?, ?, ?, ?, ?, NOW(), NOW(), NULL)
            ON DUPLICATE KEY UPDATE company_id = VALUES(company_id), dept_id = VALUES(dept_id), owner_user_id = VALUES(owner_user_id), revenue_date = VALUES(revenue_date), revenue_json = VALUES(revenue_json), revenue_type = VALUES(revenue_type), category = VALUES(category), amount = VALUES(amount), payment_method = VALUES(payment_method), handler = VALUES(handler), updated_at = NOW(), deleted_at = NULL
          `, [row.id, scope.companyId, scope.deptId, scope.ownerUserId, scope.createdBy, date, JSON.stringify(row), row.type || null, row.category || null, Number(row.amount || 0), row.paymentMethod || null, row.handler || null])
        ...
        if (input.daily !== undefined)
          await upsertHotelDailyRecord(connection, input.daily, scope)
      })
      return this.list(date, scope.companyId)
    }
    if (isDatabaseRequired())
      throw new Error('数据库为必需配置，酒店营收禁止写入本地 JSON')
    const current = await this.list(undefined, scope.companyId)
    ...
    if (input.daily !== undefined)
      await hotelDailyStore.save(input.daily, scope.companyId, scope.deptId, scope.ownerUserId)
    return this.list(date, scope.companyId)
  },
}
...
async function ensureHotelRevenueSchema(db: mysql.Pool) {
  ...
  `)
  await ensureColumn(db, 'hotel_revenue', 'company_id', 'BIGINT NOT NULL DEFAULT 1')
